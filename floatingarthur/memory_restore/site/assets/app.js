...
const api = (path, options = {}) => {
  if (!apiBase) return Promise.reject(new Error("정적 미리보기입니다. ?apiBase=https://API-ADDRESS 형식으로 Flask API 주소를 연결하세요."));
...
function assetUrl(path) { return apiBase ? new URL(path, `${apiBase}/`).toString() : path; }
...
async function initialize() {
  ...
  try {
    ...
    elements.apiStatus.textContent = configuredApiBase ? "API unavailable" : "Static preview";
    elements.uploadNote.textContent = error.message;
  }
}
